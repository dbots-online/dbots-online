...
fs.readdir("./src/servers/commands/", (err, files) => {
  ...
  console.log(`[dbots.online/servers]: ${files.length} command loaded.`);
...
serverClient.on('ready',async () => {
    console.log("[dbots.online/servers]: Bot successfully connected as "+serverClient.user.tag+".");
    serverClient.user.setPresence({ activity: { type: 'WATCHING', name: '-bump & -vote | dbots.online' }, status: "online" });
});
