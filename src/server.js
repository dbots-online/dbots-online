  ...
  module.exports = async (client) => {
    ...
    app.use(session({
      ...
      secret: "GYG*^BC^#*G&R#&G)CHEF&(GC$GF(&$FHCIHGWF(WYFGV(#HRG(&VH&GH(&$#&VGYG&VGG#*$(&VG&(#$GUV$G$G&($G&#TVG&#GY#*G^$GF$@^(GFV$^@*GFV*^*VG*V^@GF(@^G#@*V#@FG*#@G^@#(*GF^#@))))))))))))",
      resave: false,
   ...
   app.get("/login", (req, res, next) => {
    ...
    app.get("/callback", passport.authenticate("discord", { failureRedirect: "/error?code=999&message=We encountered an error while connecting." }), async (req, res) => {
        ...
        client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({dynamic: true})).setThumbnail(a.avatarURL({dynamic: true})).setColor("RED").setDescription(`[**${a.username}**#${a.discriminator}](https://dbots.online/user/${a.id}) Has been login to **DBots.online**.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
        })
        req.session.destroy(() => {
        res.json({ login: false, message: "You have been blocked from DBots.online.", logout: true })
        req.logout();
        });
        } else {
            try {
        ...
        client.users.fetch(req.user.id).then(async a => {
        client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({dynamic: true})).setThumbnail(a.avatarURL({dynamic: true})).setColor("GREEN").setDescription(`[**${a.username}**#${a.discriminator}](https://dbots.online/user/${a.id}) Has been login to **DBots.online**.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
    ...
    http.listen(3000, () => { console.log("[dbots.online]: Website running on 3000 port.")});
    ...
    */
    console.log(`
      ...
      [===========================================]
                       DBots.online
              https://github.com/dbots-online/
                   Developed by Claudette
                   Modified by AllGameRM
      [===========================================]
    ...
    sleep(1050)
    //console.clear();
    console.log('\x1b[36m%s\x1b[0m', "[dbots.online]: General routers loading...");
    sleep(500);
    ...
    /* Uptime System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[dbots.online]: Uptime system routers loading...");
    sleep(500);
    ...
    /* Profile System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[dbots.online]: Profile system routers loading...");
    sleep(500);
    ...
    /* Code Share System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[dbots.online]: Code Share system routers loading...");
    sleep(500);
    ...
    /* Botlist System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[dbots.online]: Botlist system routers loading...");
    sleep(500);
    ...
    /* Server List System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[dbots.online]: Serverlist system routers loading...");
    sleep(500);
    ...
    app.use(async (req, res, next) => {
       if(req.path.includes('/admin')) {
        if (req.isAuthenticated()) {
          if(client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(global.config.server.roles.administrator) || client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(global.config.server.roles.moderator)) {
              next();
              } else {
    ...
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[dbots.online]: Admin Panel system routers loading...");
    sleep(500);
    ...
    /* Bot System */
    console.log(" ")
    console.log('\x1b[36m%s\x1b[0m', "[dbots.online]: Bot system loading...");
