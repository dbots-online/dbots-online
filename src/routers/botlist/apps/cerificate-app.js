...
console.log("[dbots.online]: Botlist/Certificate Application router loaded.");
...
app.post("/certification/apply", global.checkAuth, async (req, res) => {
    ...
    client.channels.cache.get(channels.botlog).send(`<@${req.user.id}>`, new discord.MessageEmbed().setColor("YELLOW").setTitle("Request Certificate").setDescription(`User **${req.user.username}** requested a certificate for her bot named **${botdata.username}**.`))
});
