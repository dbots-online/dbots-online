...
console.log("[dbots.online]: Botlist/Add Bot router loaded.");
    ...
    app.post("/addbot", global.checkAuth, async (req,res) => {
      ...
      client.channels.cache.get(channels.botlog).send(`<@${req.user.id}> added **${a.tag}**`, new discord.MessageEmbed().setColor("PURPLE").setTitle("Bot Added").setDescription(`<@${req.user.id}> added **${a.tag}**\n**Bot:**\n\`${a.tag}\` (\`${rBody['botID']}\`) \n**Owner:**\n<@${req.user.id}> (\`${req.user.id}\`)`).setTimestamp())
