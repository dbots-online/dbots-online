...
console.log("[dbots.online]: Admin/Botlist/Confirm Bot router loaded.");
...
app.get("/admin/confirm/:botID", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(req.params.botID).then(bota => {
        client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}>`,new   discord.MessageEmbed()
.setColor("GREEN")
.setAuthor("DBots.Online", client.user.displayAvatarURL({ Dynamic: true }))
.setDescription(`
Your bot **${bota.tag}** has been approved on DBots.online! 🎉
`).setTimestamp())
        client.users.cache.get(botdata.ownerID).send(new MessageEmbed().setColor("GREEN").setAuthor("DBots.Online", client.user.displayAvatarURL({ Dynamic: true })).setDescription(`
Your bot **${bota.tag}** has been approved on DBots.online! 🎉
`))
    });
